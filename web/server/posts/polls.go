...
var pollsRoute = web.Route{
	Pattern: res.UrlPostsPolls,
	Handler: func(r *web.Response) {
		...
		posts, err := profile.GetPollsPosts(userPkHash, uint(offset))
		if err != nil {
			r.Error(jerr.Get("error getting polls posts", err), http.StatusInternalServerError)
			return
		}
